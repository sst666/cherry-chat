...
export default function ChatArea() {
  ...
  const allShowableModels = [
    ...state.recentModels,
    ...state.models.filter((m) => !state.recentModels.includes(m)),
  ];
  ...
  return (
    <div className="flex flex-col flex-1 min-w-0 h-full bg-white">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-b border-[#e5e7eb] shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-sm font-medium text-[#111827] truncate">
            {state.conversations.find((c) => c.id === state.currentConvId)?.title ?? 'New Conversation'}
          </h1>
        </div>
        <ModelSelector />
      </div>
      ...
      {allShowableModels.length > 0 && (
        <div className="flex items-center gap-2 px-4 py-2 border-b border-[#e5e7eb] bg-[#f9fafb] overflow-x-auto shrink-0"
          style={{ scrollbarWidth: 'none' } as React.CSSProperties}>
          {state.recentModels.length > 0 && (
            <>
              <span className="text-[10px] text-[#9ca3af] shrink-0 font-medium">最近</span>
              {state.recentModels.map((m) => (
                <button
                  key={`recent-${m}`}
                  ...
                  style={state.config.model === m ? { background: 'linear-gradient(135deg, #4f46e5, #6366f1)' } : {}}
                >
                  {m}
                </button>
              ))}
              {state.models.filter((m) => !state.recentModels.includes(m)).length > 0 && (
    ...
    <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-[#6b7280] max-w-xs">
          Start a conversation, attach images or PDFs, and explore ideas with AI.
        </p>
      </div>
