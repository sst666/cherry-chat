...
export default function MessageBubble({ message }: Props) {
  ...
  return (
    <div
      className={`group flex items-start gap-2 sm:gap-3 py-2 px-2 sm:px-0 ${
          ...
          <div className="flex flex-wrap gap-2 mb-1">
            {message.images.map((img, i) => (
              <img
                ...
                src={img}
                alt={`attachment-${i}`}
                className="max-w-[140px] sm:max-w-[200px] max-h-[140px] sm:max-h-[200px] rounded-lg border border-[#e5e7eb] object-cover"
              />
            ))}
          </div>
        )}
